import { Alert, AlertColor, Button, ButtonBase, Card, CardContent, CardMedia, FormHelperText, Icon, MenuItem, Radio, RadioGroup, Select, Stack, TextField, Typography } from '@mui/material'
	...
	const submit = async() => {
		...
		console.log('ey')
			const {
				...
				equipments,survival,emergencyradio,	personsCollection,remarks,active,other, approved, id
					...
					<CardContent sx = {{display: 'flex', flexDirection: 'column'}}>
						...
						<Stack direction="column" spacing={2} marginTop={2}>
								...
								<TextField
										...
										disabled = {isDisable}
										error = {submitted && personscollection.length === 0}
										helperText = {submitted && personscollection.length === 0 && 'Field must not be empty'}
								/>
						</Stack>
						<br/>
						...
						<Stack direction="column" spacing={2} marginTop={2}>
								<FormHeader inputLabel = 'Approval' />
							    <Select 
								value={form[0].approved ? 'YES' : 'NO'}
								onChange={(e) => {
									const isApprove = e.target.value == 'YES' ? true: false
									setform((prev) => [{
										...prev[0],
										approved: isApprove
									}])
